...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    // Esses argumentos devem bater com o construtor padrão da classe MovieMinDTO(obj.id, obj.name)
    @Query(value =
            "SELECT new com.devsuperior.uri2611.dto.MovieMinDTO(obj.id, obj.name) "
                    + "FROM Movie obj "
                    + "WHERE obj.genre.description = :genreName")
        // Com o relacionamento para um, não preciso do inner join no jpql, basta navegar entre os objetos(Movie(private Genre genre) -> Genre(atributo: description))
